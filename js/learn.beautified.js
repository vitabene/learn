...
function makeVisualConnection() {
    ...
    var lineBeginning = {
        x: rect1.right,
        y: rect1.top + height1 / 2
    };
    var lineEnd = {
        x: rect2.left,
        y: rect2.top + height2 / 2
    };
    ...
    var linePosition = {
        x: lineBeginning.x + X / 2 - lineWidth / 2 - outerRect.left,
        y: lineBeginning.y + Y / 2 - outerRect.top + 3
    };
...
function makeConnection(button) {
    ...
    if (button.className.indexOf("connect") === -1 || button.className.indexOf("mistake") !== -1) button.className = "connect"; else button.className = "";
    ...
    var lines = byClassName("line");
    if (lines.length !== pairsInUse.length) byId("checkbutton").disabled = true; else byId("checkbutton").disabled = false;
}
...
function shuffleArray(a) {
    for (var j, x, i = a.length; i; j = Math.floor(Math.random() * i), x = a[--i], a[i] = a[j], 
    a[j] = x) ;
    return a;
}
